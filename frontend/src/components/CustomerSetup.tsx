...
export const CustomerSetup: React.FC<CustomerSetupProps> = ({
  ...
  const handlePreferencesSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createCustomerMutation.mutateAsync({
        ...
        preferences: {
          ...preferences,
          dietaryRestrictions: preferences.dietaryRestrictions?.filter(r => r.trim()),
  ...
  const addDietaryRestriction = (restriction: string) => {
    if (restriction && !preferences.dietaryRestrictions?.includes(restriction)) {
      setPreferences(prev => ({
        ...prev,
    ...
    return (
      <div className={`max-w-md mx-auto p-6 bg-white border border-gray-200 rounded-lg shadow-sm ${className}`}>
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-gray-900">Welcome to AI-Mi!</h2>
          <p className="text-gray-600 mt-2">Let's get you set up for the best dining experience</p>
        </div>
  ...
  return (
    <div className={`max-w-lg mx-auto p-6 bg-white border border-gray-200 rounded-lg shadow-sm ${className}`}>
      <div className="text-center mb-6">
        <Coffee className="h-12 w-12 mx-auto text-green-500 mb-4" />
        <h2 className="text-2xl font-bold text-gray-900">Your Preferences</h2>
        <p className="text-gray-600 mt-2">Help us personalize your dining experience</p>
      </div>
      ...
      <form onSubmit={handlePreferencesSubmit} className="space-y-6">
        ...
        <div className="flex space-x-3">
          ...
          >
            {createCustomerMutation.isLoading ? 'Creating...' : 'Start Chatting'}
          </button>
        </div>
      </form>
    </div>
